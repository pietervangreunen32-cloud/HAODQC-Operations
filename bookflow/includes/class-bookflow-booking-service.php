...
class BookFlow_Booking_Service {
	...
	/**
	 * Edits an existing appointment in place — the shop-side "make it
	 * editable" screen calls this rather than create_booking(), since an
	 * edit must never count against the plan's monthly booking cap and
	 * must not conflict with its own current time slot / item reservations
	 * when nothing about them actually changed.
	 *
	 * Validates the new date/time/items *before* touching any existing
	 * row: if validation fails, the appointment is left completely
	 * untouched rather than losing its current reservations while the
	 * update itself is rejected.
	 *
	 * @param int   $appointment_id
	 * @param array $request Same shape as create_booking(), minus $source.
	 * @return int|WP_Error Appointment ID, or WP_Error explaining why it failed.
	 */
	public static function update_booking( $appointment_id, array $request ) {
		...
		// Validated against the appointment's *current* reservations still
		// in place — excluding this appointment's own ID is what stops
		// "keep the same time and items" from looking like a conflict with
		// itself. Nothing is written until this passes.
		...
		// Simplest correct way to reconcile companions/items on an edit is
		// to replace them wholesale rather than diff old vs. new — safe now
		// that validation above has already passed.
		BookFlow_DB_Reservations::delete_for_appointment( $appointment_id );
		BookFlow_DB_Companions::delete_for_appointment( $appointment_id );
		...
		/**
		 * Fires after an existing appointment is edited, appointment ID as
		 * the only argument — separate from bookflow_booking_created so
		 * anything listening for genuinely new bookings (e.g. the
		 * ReviewLoop bridge's completed-appointment hand-off) never
		 * double-fires on an edit.
		 */
