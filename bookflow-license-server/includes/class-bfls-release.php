<?php
/**
 * Stores each uploaded BookFlow plugin build (version, changelog, and the
...
class BFLS_Release {
	...
	public static function uploads_dir() {
		...
		return trailingslashit( $upload_dir['basedir'] ) . 'bookflow-releases';
	}

	public static function get_latest() {
		global $wpdb;
		$table = BFLS_DB::releases_table();
	...
	public static function create_from_upload( $version, $changelog, $min_wp, $tested_wp, $uploaded_file ) {
		...
		if ( '' === $version || ! preg_match( '/^\d+\.\d+(\.\d+)?$/', $version ) ) {
			return new WP_Error( 'bfls_bad_version', __( 'Version must look like 1.7.0.', 'bookflow-license-server' ) );
		}

		if ( empty( $uploaded_file['tmp_name'] ) || 'zip' !== strtolower( pathinfo( $uploaded_file['name'], PATHINFO_EXTENSION ) ) ) {
			return new WP_Error( 'bfls_bad_file', __( 'Please upload the plugin as a .zip file.', 'bookflow-license-server' ) );
		}
		...
		if ( ! @move_uploaded_file( $uploaded_file['tmp_name'], $dest_path ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
			return new WP_Error( 'bfls_upload_failed', __( 'Could not save the uploaded file.', 'bookflow-license-server' ) );
		}

		global $wpdb;
		$wpdb->replace(
			BFLS_DB::releases_table(),
			array(
				'version'    => $version,
				'changelog'  => $changelog,
				'file_path'  => $dest_path,
	...
	public static function delete( $release_id ) {
		...
		global $wpdb;
		$wpdb->delete( BFLS_DB::releases_table(), array( 'id' => $release_id ), array( '%d' ) );
	}
	...
	public static function download_url( $release_id, $license_key ) {
		...
		return add_query_arg(
			...
			rest_url( 'bookflow-license/v1/update-download' )
		);
	}
