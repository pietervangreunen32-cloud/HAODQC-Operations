<?php
/**
 * The REST API the BookFlow plugin's client (class-bookflow-license.php)
...
class BFLS_Api {
	...
	public function register_routes() {
		...
		register_rest_route( 'bookflow-license/v1', '/activate', array_merge( $args, array( 'callback' => array( $this, 'activate' ) ) ) );
		register_rest_route( 'bookflow-license/v1', '/deactivate', array_merge( $args, array( 'callback' => array( $this, 'deactivate' ) ) ) );
		register_rest_route( 'bookflow-license/v1', '/validate', array_merge( $args, array( 'callback' => array( $this, 'validate' ) ) ) );
	...
	public function activate( WP_REST_Request $request ) {
		$result = BFLS_License::handle_activate_request(
	...
	public function deactivate( WP_REST_Request $request ) {
		$result = BFLS_License::handle_deactivate_request(
			sanitize_text_field( $request->get_param( 'license_key' ) ),
			esc_url_raw( $request->get_param( 'site_url' ) )
		);
		return new WP_REST_Response( $result, 200 );
	...
	public function validate( WP_REST_Request $request ) {
		$result = BFLS_License::handle_validate_request(
			sanitize_text_field( $request->get_param( 'license_key' ) )
		);
		return new WP_REST_Response( $result, 200 );
	}
	...
	public function update_check( WP_REST_Request $request ) {
		$result = BFLS_License::handle_update_check_request(
			sanitize_text_field( $request->get_param( 'license_key' ) ),
			esc_url_raw( $request->get_param( 'site_url' ) ),
	 ...
	 * license key, and a short expiry window (see BFLS_Release).
	 */
	public function update_download( WP_REST_Request $request ) {
		...
		if ( ! BFLS_Release::verify_token( $release_id, $license_key, $expires, $token ) ) {
			return new WP_REST_Response( array( 'message' => 'Invalid or expired download link.' ), 403 );
		}

		$release = BFLS_Release::get( $release_id );
		...
		header( 'Content-Disposition: attachment; filename="bookflow-' . $release->version . '.zip"' );
